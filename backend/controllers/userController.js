...
// ⭐ Toggle blog in favourites (add if not present, remove if present)
const toggleFavourite = async (req, res) => {
  try {
    ...
    const isFavorited = user.favourites.includes(blogId);
    ...
    res.status(500).json({
      message: "Server error while toggling favourites",
      error: error.message,
    });
  }
};
